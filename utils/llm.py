    ...
    def __init__(
        ...
        **extra_params,
        ...
        Args:
            ...
            default_headers: Extra HTTP headers (e.g. anthropic-version for Anthropic API).
...
def create_llm_client(config: Dict[str, Any]) -> LLMClient:
    ...
    framework_keys = {"provider", "base_url", "api_key", "model", "timeout", "retry_times", "retry_delay", "default_headers"}

    api_key = api_config.get("api_key", "EMPTY")
    # Support ${ENV_VAR} substitution in api_key
    if isinstance(api_key, str) and api_key.startswith("${") and api_key.endswith("}"):
        env_var = api_key[2:-1]
        api_key = os.environ.get(env_var, "EMPTY")

    base_url = api_config.get("base_url", "https://api.openai.com/v1")
    provider = api_config.get("provider", "openai")

    # Anthropic's API requires the anthropic-version header when accessed via OpenAI SDK
    default_headers: Optional[Dict[str, str]] = api_config.get("default_headers")
    if default_headers is None and (provider == "anthropic" or "anthropic.com" in str(base_url)):
